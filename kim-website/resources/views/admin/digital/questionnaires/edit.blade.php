    ...
    <div class="row">
        ...
        <div class="col-lg-8">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="type" class="form-select @error('type') is-invalid @enderror">
                                    ...
                                    <option value="anxiety" {{ old('type', $questionnaire->type) == 'anxiety' ? 'selected' : '' }}>Anxiety</option>
                                    <option value="depression" {{ old('type', $questionnaire->type) == 'depression' ? 'selected' : '' }}>Depression</option>
                                    <option value="personality" {{ old('type', $questionnaire->type) == 'personality' ? 'selected' : '' }}>Personality</option>
                                    <option value="general" {{ old('type', $questionnaire->type) == 'general' ? 'selected' : '' }}>General</option>
                                </select>
                                @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div id="aiSettings" style="{{ old('ai_enabled', $questionnaire->ai_enabled ?? true) ? '' : 'display: none;' }}">
                        <div class="form-group">
                            <label class="form-label">Persona AI</label>
                            <select name="ai_persona" class="form-select @error('ai_persona') is-invalid @enderror">
                                <option value="psikolog" {{ old('ai_persona', $questionnaire->ai_persona ?? 'psikolog') == 'psikolog' ? 'selected' : '' }}>
                                    🧠 Psikolog Profesional
                                </option>
                                <option value="konselor" {{ old('ai_persona', $questionnaire->ai_persona) == 'konselor' ? 'selected' : '' }}>
                                    💬 Konselor
                                </option>
                                <option value="coach" {{ old('ai_persona', $questionnaire->ai_persona) == 'coach' ? 'selected' : '' }}>
                                    🎯 Life Coach
                                </option>
                                <option value="mentor" {{ old('ai_persona', $questionnaire->ai_persona) == 'mentor' ? 'selected' : '' }}>
                                    👨‍🏫 Mentor Akademik
                                </option>
                                <option value="hr" {{ old('ai_persona', $questionnaire->ai_persona) == 'hr' ? 'selected' : '' }}>
                                    👔 HR Professional
                                </option>
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <small class="text-muted">
                                Contoh: "Fokus pada aspek akademik mahasiswa", "Berikan rekomendasi yang bisa dilakukan sendiri di rumah", "Gunakan bahasa yang mudah dipahami remaja"
                            </small>
                            @error('ai_context')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="alert alert-warning" style="margin-bottom: 0;">
                            ...
                            <br>Setiap analisis AI menggunakan API Claude yang memiliki biaya. Estimasi ~Rp 500-1500 per analisis tergantung kompleksitas.
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <a href="{{ route('admin.digital.questions.index', ['questionnaire_id' => $questionnaire->id]) }}" 
                       class="btn btn-outline-primary w-100 mb-2">
                        <i class="fas fa-question-circle"></i> Kelola Pertanyaan
                    </a>
                    <a href="{{ route('admin.digital.dimensions.index', ['questionnaire_id' => $questionnaire->id]) }}" 
                       class="btn btn-outline-info w-100 mb-2">
                        <i class="fas fa-layer-group"></i> Kelola Dimensi
                    </a>
                    <a href="{{ route('admin.digital.responses.index', ['questionnaire_id' => $questionnaire->id]) }}" 

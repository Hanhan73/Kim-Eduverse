    ...
    <form id="quizForm" action="{{ route('edutech.student.quiz.submit', $currentQuiz->id) }}" method="POST">
        ...
        <div style="background: white; padding: 15px; border-radius: 10px; margin-bottom: 20px;">
            ...
            <div style="width: 100%; height: 8px; background: #e0e0e0; border-radius: 10px; overflow: hidden;">
                <div id="progressBar" style="width: 0%; height: 100%; background: linear-gradient(90deg, var(--primary), var(--secondary)); transition: width 0.3s ease;"></div>
            </div>
        </div>
        ...
        <div class="question-navigator">
            @php
                // Ambil urutan soal dari attempt (jika ada randomize)
                $questionOrder = $currentAttempt->question_order 
                    ? json_decode($currentAttempt->question_order) 
                    : $currentQuiz->questions->pluck('id')->toArray();
                
                // Urutkan questions sesuai question_order
                $orderedQuestions = collect($questionOrder)->map(function($qId) use ($currentQuiz) {
                    return $currentQuiz->questions->firstWhere('id', $qId);
                })->filter();
            @endphp
